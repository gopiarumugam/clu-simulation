...
public class Cryptor {
	...
	byte[] FS_CRYPT_KEY = new byte[] { 0x011, (byte) 0xff, 0x33, (byte) 0xee, (byte) 0x55, (byte) 0xdd, (byte) 0x77,
			(byte) 0xbb };
	...
	public void init() throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException {
		String k = approperties.getProperty("facesix.cryptkey");
		if (k != null) {
			try {
				FS_CRYPT_KEY = k.getBytes("UTF-8");
			} catch (Exception e) {
				LOG.warn("Failed to use cryptkey [%s]. Continuing with default key. Try keys with 8 chars.", k, e);
			}
		}
		syskey = new SecretKeySpec(FS_CRYPT_KEY, "DES");
