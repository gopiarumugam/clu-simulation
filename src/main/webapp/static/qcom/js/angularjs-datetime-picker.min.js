  ...
  var DatetimePickerCtrl = function($compile, $document) {
    ...
    this.openDatetimePicker = function(options) {
      ...
      if(bcr.width > datePickerElBcr.width){
        ...
        datetimePickerEl.style.left= (bcr.left + bcr.width - datePickerElBcr.width + scrollX) + 'px';
      } else { 
    	  
        datetimePickerEl.style.left= (bcr.left + scrollX) + 'px';
      }
      var scrollY = 0;
	  if(window.scrollX != undefined ){
      ...
      if (bcr.top < 300 || window.innerHeight - bcr.bottom > 300) {
        datetimePickerEl.style.top = (bcr.bottom + scrollY) + 'px';
      } else {
    	  
        datetimePickerEl.style.top = (bcr.top - datePickerElBcr.height + scrollY) + 'px';
      }
      ...
      link: function(scope, element, attrs, ctrl) {
        ...
        scope.$watch(attrs.ngModel, function(value) {
          ...
          if( attrs.hasOwnProperty('futureOnly') ){
        	  ...
        	  if(dateDiff <= 0){
        		  ctrl.$setValidity('future-only', true);
        	  }else{
        		  ctrl.$setValidity('future-only', false);
        	  } 
          }
          
          if( attrs.hasOwnProperty('mindate') ){ 
        	  var dateRange = new Date(attrs.mindate);
        	  var dateDiff = dateRange - date;
        	  console.log(dateDiff);
        	  if(dateDiff <= 0){
        		  ctrl.$setValidity('date-range', true);
        	  }else{
        		  ctrl.$setValidity('date-range', false);
        	  }
